 ...
 * @status
 */
public class test_las implements TalendJob {
	...
	private final String jobName = "test_las";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(test_las.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		final static byte[] commonByteArrayLock_LOCAL_test_las = new byte[0];
		static byte[] commonByteArray_LOCAL_test_las = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_LOCAL_test_las.length) {
					if (length < 1024 && commonByteArray_LOCAL_test_las.length == 0) {
						commonByteArray_LOCAL_test_las = new byte[1024];
					} else {
						commonByteArray_LOCAL_test_las = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_LOCAL_test_las, 0, length);
				strReturn = new String(commonByteArray_LOCAL_test_las, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_LOCAL_test_las, 0, length);
				strReturn = new String(commonByteArray_LOCAL_test_las, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_LOCAL_test_las) {
...
///////////////////////    			
	...
	public static void main(String[] args) {
		final test_las test_lasClass = new test_las();

		int exitCode = test_lasClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = test_las.class.getClassLoader()
					.getResourceAsStream("local/test_las_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = test_las.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : test_las");
		}
