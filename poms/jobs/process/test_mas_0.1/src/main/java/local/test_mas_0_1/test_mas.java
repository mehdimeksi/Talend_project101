...
/**
 * Job: test_mas Purpose: <br>
 * Description: <br>
 * 
 * @author mehdimeksi@gmail.com
 * @version 8.8.8.20250604_1135-SNAPSHOT
 * @status
 */
public class test_mas implements TalendJob {
	...
	private final String jobName = "test_mas";
	private final String projectName = "LOCAL";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(test_mas.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		final static byte[] commonByteArrayLock_LOCAL_test_mas = new byte[0];
		static byte[] commonByteArray_LOCAL_test_mas = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_LOCAL_test_mas.length) {
					if (length < 1024 && commonByteArray_LOCAL_test_mas.length == 0) {
						commonByteArray_LOCAL_test_mas = new byte[1024];
					} else {
						commonByteArray_LOCAL_test_mas = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_LOCAL_test_mas, 0, length);
				strReturn = new String(commonByteArray_LOCAL_test_mas, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_LOCAL_test_mas, 0, length);
				strReturn = new String(commonByteArray_LOCAL_test_mas, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_LOCAL_test_mas) {
	...
	public void tRowGenerator_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				util_tLogRow_1.addRow(new String[] { "name", "lastname", "city", });
...
///////////////////////		
					...
					if (row1.city != null) { //
						row_tLogRow_1[2] = String.valueOf(row1.city);
...
///////////////////////    			
	...
	public static void main(String[] args) {
		final test_mas test_masClass = new test_mas();

		int exitCode = test_masClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = test_mas.class.getClassLoader()
					.getResourceAsStream("local/test_mas_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = test_mas.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : test_mas");
		}
...
/************************************************************************************************
 * 46184 characters generated by Talend Open Studio for ESB on the 23 octobre
 * 2025 à 23:13:10 CEST
 ************************************************************************************************/